...
const refreshTime = computed(() =>
  response.value ? new Date(response.value.timestamp).toLocaleString() : '-',
)
const totalValue = computed(() =>
  response.value
    ? response.value.items.reduce((sum, item) => sum + item.price, 0)
    : 0,
)
const averagePrice = computed(() =>
  response.value && response.value.items.length > 0
    ? totalValue.value / response.value.items.length
    : 0,
)
...
<template>
  <v-app>
    <v-app-bar color="white" elevation="1">
      <v-app-bar-title class="font-weight-bold">Portfolio Tracker</v-app-bar-title>
        ...
        <v-row>
          <v-col cols="12" md="8">
            ...
            <p class="text-subtitle-1 text-medium-emphasis">
              Monitor portfolio snapshot data from your backend test endpoint.
            </p>
          </v-col>
